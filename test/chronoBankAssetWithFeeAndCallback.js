...
var ChronoBankAssetWithFeeAndCallback = artifacts.require("./ChronoBankAssetWithFeeAndCallback.sol");
...
contract('ChronoBankAssetWithFeeAndCallback', function(accounts) {
  ...
  before('setup others', function(done) {
    ...
    stub = instance;
    ChronoBankAssetWithFeeAndCallback.new().then(function(instance) {
  ...
  it('should add and notify listeners', async () => {
    ...
    await chronoBankPlatform.reissueAsset(SYMBOL, amount);
    await chronoBankPlatform.setProxy(chronoBankAssetProxy.address, SYMBOL);
    await chronoBankAsset.setupFee(feeAddress, feePercent);

    let multiEventsHistory = await MultiEventsHistory.deployed();

    let listener1 = await ERC223ReceiverTest.new(MultiEventsHistory.address);
    multiEventsHistory.authorize(listener1.address);
    let listener2 = await ERC223ReceiverTest.new(MultiEventsHistory.address);
    multiEventsHistory.authorize(listener2.address);
    let listener3 = await ERC223ReceiverTest.new(MultiEventsHistory.address);
    multiEventsHistory.authorize(listener3.address);
    let listener4 = await ERC223ReceiverTest.new(MultiEventsHistory.address);
    multiEventsHistory.authorize(listener4.address);

    await chronoBankAsset.addListener(listener1.address, "1");
    await chronoBankAsset.addListener(listener2.address, "2");
    await chronoBankAsset.addListener(listener3.address, "3");
    await chronoBankAsset.addListener(listener4.address, "4");


     let transferTx = await chronoBankAssetProxy.transfer(spender, amount);

     let transferEvents = eventsHelper.extractEvents(transferTx, "Transfer");
     assert.equal(transferEvents.length, 1);
     assert.equal(transferEvents[0].args.from.valueOf(), holder);
     assert.equal(transferEvents[0].args.to.valueOf(), spender);
     assert.equal(transferEvents[0].args.value.valueOf(), amount);
